...
let endDate = "2023-02-06";
let perPage = 50;
let category = "sports"
let apiUrlSeatGeek1 = `https://api.seatgeek.com/2/events?lat=${lat}&lon=${lon}&range=${range}&datetime_utc.gte=${startDate}&datetime_utc.lte=${endDate}&per_page=${perPage}&sort=datetime_local.asc&taxonomies.name=${category}&client_id=${apiKeySeatGeek}`; 
fetch(apiUrlSeatGeek1)
.then(function (response) {
  if (response.ok===false) { // When there's an error, show the alert message below and do not continue subsequent executions
  // alert("Please check the city name again. It may be incorrect.");
...
let apiUrlSeatGeek2 = `https://api.seatgeek.com/2/taxonomies?client_id=${apiKeySeatGeek}`; 
